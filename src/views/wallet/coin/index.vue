<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="币种精度" prop="coinDecimals">
        <el-input
          v-model="queryParams.coinDecimals"
          placeholder="请输入币种精度"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="币种确认数" prop="coinConfirm">
        <el-input
          v-model="queryParams.coinConfirm"
          placeholder="请输入币种确认数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="最小归集数量" prop="minCollectAmount">
        <el-input
          v-model="queryParams.minCollectAmount"
          placeholder="请输入最小归集数量"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="coinList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="币种精度" align="center" prop="coinDecimals" />
      <el-table-column label="币种确认数" align="center" prop="coinConfirm" />
      ...
      <el-table-column label="最小归集数量" align="center" prop="minCollectAmount" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="币种精度" prop="coinDecimals">
          <el-input v-model="form.coinDecimals" placeholder="请输入币种精度" />
        </el-form-item>
        <el-form-item label="币种确认数" prop="coinConfirm">
          <el-input v-model="form.coinConfirm" placeholder="请输入币种确认数" />
        </el-form-item>
        <el-form-item label="创建时间" prop="ctime">
          <el-date-picker clearable
            v-model="form.ctime"
        ...
        <el-form-item label="最小归集数量" prop="minCollectAmount">
          <el-input v-model="form.minCollectAmount" placeholder="请输入最小归集数量" />
        </el-form-item>
        <el-form-item label="主链类型" prop="chainType">
...
<script>
import { listCoin, getCoin, delCoin, addCoin, updateCoin } from "@/api/wallet/coin";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        coinConfirm: [
          { required: true, message: "币种确认数不能为空", trigger: "blur" }
        ],
        minCollectAmount: [
          { required: true, message: "最小归集数量不能为空", trigger: "blur" }
        ],
        type: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除币种编号为"' + ids + '"的数据项？').then(function() {
        return delCoin(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wallet/coin/export', {
        ...this.queryParams
